...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        private Dictionary<DMSType, ResourceDescription> CreateResourcesToInsert()
        {
            ...
            //SetTransformerEndReferences(updates);
        ...
        private void SetTerminalReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.TERMINAL].Properties.Count; i++)
            {
                if (updates[DMSType.TERMINAL].Properties[i].Id == ModelCode.TERMINAL_CONNECTIVITYNODE)
                {
                ...
                {
                    updates[DMSType.TERMINAL].Properties[i].SetValue(updates[DMSType.POWERTR].Id);
                }
            }
        }

        private void SetTransformerEndReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.POWTREND].Properties.Count; i++)
            {
                if (updates[DMSType.POWTREND].Properties[i].Id == ModelCode.TRANSFORMEREND_TERMINAL)
                {
                    updates[DMSType.POWTREND].Properties[i].SetValue(updates[DMSType.TERMINAL].Id);
                }

                if (updates[DMSType.POWTREND].Properties[i].Id == ModelCode.TRANSFORMEREND_RATIOTAPCHNG)
                {
                    updates[DMSType.POWTREND].Properties[i].SetValue(updates[DMSType.RATIOTAPCHANGER].Id);
                }

                if (updates[DMSType.POWTREND].Properties[i].Id == ModelCode.POWTREND_POWTR)
                {
                    updates[DMSType.POWTREND].Properties[i].SetValue(updates[DMSType.POWERTR].Id);
                }
            }
        }
